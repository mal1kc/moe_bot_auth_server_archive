...
class PaketIcerik(Base):
    ...
    def __repr__(self):
        return "<PaketIcerik(p_icerikId='%s', p_icerikAdi='%s')>" % (self.p_icerikId, self.p_icerikAdi)
...
def main():
    ...
    session = Session()
    for i in range(35):
        session.add(PaketIcerik(p_icerikId=i, p_icerikAdi="".join([ascii_letters[randint(0, 51)] for i in range(256)])))
    session.commit()
